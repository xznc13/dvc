DELIMITER $$
DROP PROCEDURE IF EXISTS roverNumRecsInUAT;
CREATE PROCEDURE roverNumRecsInUAT(IN in_core_system text, in_process_date date, out out_result int)
BEGIN
-- -----------------------------------------------------------------------------------------------------
-- (C) Red Hound Limited 2015
-- -----------------------------------------------------------------------------------------------------
--
-- Title	ROVER - Number of records in UAT
--
-- DB:		MARIADB
--
-- Purpose	Retrieve the number of records in the UAT tables for systems that are switched on
--
...
-- Logging variables
-- -----------------------------------------------------
DECLARE v_logging_process TEXT DEFAULT 'roverNumRecsInUAT';
...
DECLARE db_cursor_core_systems CURSOR FOR  
 SELECT core_uat_table, core_system
   FROM core_systems
