DELIMITER $$
DROP PROCEDURE IF EXISTS roverResultsNumRecsDiffUncat;
CREATE PROCEDURE roverResultsNumRecsDiffUncat(IN in_core_system text, out out_result int)
BEGIN
-- -----------------------------------------------------------------------------------------------------
-- (C) Red Hound Limited 2015
-- -----------------------------------------------------------------------------------------------------
--
-- Title	ROVER - Number of records with no differences
--
-- DB:		MARIADB
--
-- Purpose	Retrieve the number of records with no differences for systems that are switched on
--
...
-- Logging variables
-- -----------------------------------------------------
DECLARE v_logging_process TEXT DEFAULT 'roverResultsNumRecsDiffUncat';
...
loop_core_systems: LOOP 
  ...
  FETCH db_cursor_core_systems INTO v_core_prod_table, v_core_uat_table, v_core_system ;
	...
	if in_core_system = '' or
	  ...
	  SET v_sql_command = concat(v_sql_command,'     and tbl_resu.core_system = ''',v_core_system,'''');
      SET v_sql_command = concat(v_sql_command,'     and not exists (select 1 from categorisation_rules tbl_cat where tbl_cat.classification_name = tbl_class.name))');
      SET v_sql_delim = ' + ';
