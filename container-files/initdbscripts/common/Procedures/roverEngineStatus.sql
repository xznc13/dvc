DELIMITER $$
DROP PROCEDURE IF EXISTS roverEngineStatus;
CREATE PROCEDURE roverEngineStatus()
BEGIN
-- -----------------------------------------------------------------------------------------------------
-- (C) Red Hound Limited 2015
-- -----------------------------------------------------------------------------------------------------
--
-- Title	ROVER - Present the engine status
--
-- DB:		MARIADB
--
-- Purpose	Pull the engine status from the logging table
--
...
-- Logging variables
-- -----------------------------------------------------
DECLARE v_logging_process TEXT DEFAULT 'roverEngineStatus';
...
SET v_sql_command = concat (v_sql_command,'  where id >= (select max(id) from logging where process = ''roverEngineControl'' and log_details = ''Started'') ');
SET v_sql_command = concat (v_sql_command,'    and process in (''roverEngineControl'',''roverComparisonEngine'') ');
